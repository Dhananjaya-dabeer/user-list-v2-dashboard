...
const MultiLevelMenu = ({
  ...
  const activeRoute = path => '' === path ? 1 : 0; // handle navigate to another route or page
    ...
    return data.map((item, index) => {
      ...
      if (item.children) {
        ...
        return <Box key={index}>
            <NavItemButton key={item.name} className="navItem" active={activeRoute(item.path)} onClick={() => console.log('Nav item click')}>
